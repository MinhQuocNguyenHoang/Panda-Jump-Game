...
static void qrcode_build_bitmap() {
  ...
  qrcode_initText(&qrcode, qrcodeData, QRCODE_VERSION, 0,
                  "https://github.com/MinhQuocNguyenHoang/Panda-Jump-Game.git");
...
public:
  int x0, x1;
  int distance() { return (x1 - x0); }
};

void view_scr_qrcode() {
  view_render.drawBitmap(QRCODE_AXIS_X, QRCODE_AXIS_Y, qrcode_bitmap,
                         qrcode_bitmap_size, qrcode_bitmap_size, WHITE);
...
void scr_qrcode_handle(ak_msg_t *msg) {
  ...
  case AC_DISPLAY_BUTON_MODE_PRESSED: {
    APP_DBG_SIG("AC_DISPLAY_BUTON_%s_PRESSED\n",
                ...
                : msg->sig == AC_DISPLAY_BUTON_DOWN_PRESSED ? "DOWN"
                                                            : "UP");

...
int main_app() {
  ...
  button_init(&btn_up, 10, BUTTON_UP_ID, io_button_up_init, io_button_up_read,
              btn_up_callback);
  ...
  flash_read(APP_FLASH_AK_DBG_FATAL_LOG_SECTOR,
             reinterpret_cast<uint8_t *>(&app_fatal_log), sizeof(fatal_log_t));
...
#if defined(TASK_MBMASTER_EN)
  /* modbus rtu init*/
  uint8_t flag_init = eMBMSerialInitExt(&xMBMMaster, MB_RTU, MBM_SERIAL_PORT,
                                        MBM_SERIAL_BAUDRATE, MB_PAR_NONE, 1);
    ...
    if (plink_hal_rev_byte(c) == LINK_HAL_IGNORED) {
      ...
      if (shell.index < SHELL_BUFFER_LENGHT - 1) {
        ...
        if (c == '\r' || c == '\n') { /* linefeed */
          ...
          task_post_common_msg(AC_TASK_SHELL_ID, AC_SHELL_LOGIN_CMD,
                               (uint8_t *)&shell.data[0], shell.index + 2);
...
void app_start_timer() {
  ...
  timer_set(AC_TASK_FW_ID, FW_CHECKING_REQ, FW_UPDATE_REQ_INTERVAL,
            TIMER_ONE_SHOT);
  timer_set(AC_TASK_DISPLAY_ID, AC_DISPLAY_INITIAL, AC_DISPLAY_INITIAL_INTERVAL,
            TIMER_ONE_SHOT);
}
 ...
 */
void app_power_on_reset() { sys_soft_reboot_counter = 0; }
